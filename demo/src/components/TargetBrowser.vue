...
function handleHlink(event) {
  if (
    ...
    event.target.href.startsWith("hlink://")
  ) {
  ...
  setTimeout(() => {
    if (!event.query.length) {
      ...
      filteredElements.value = settings.modelElements.filter((elem) => {
        return (
          elem.name.toLowerCase().includes(event.query.toLowerCase()) ||
          elem.uuid.includes(event.query)
        );
      });
    }
  }, 250);
...
<template>
  <div class="h-full overflow-auto">
    <div class="searchbar sticky top-0 flex flex-grow">
      <Breadcrumb
        :model="settings.tableBreadcrumbs"
      ...
      <InputGroup class="flex-1 p-2">
        <AutoComplete
          ...
          :invalid="!settings.targetUUID"
          @complete="search"
          :completeOnFocus="true"
          @keydown.enter="loadTarget"
          @option-select="loadTarget"
        >
